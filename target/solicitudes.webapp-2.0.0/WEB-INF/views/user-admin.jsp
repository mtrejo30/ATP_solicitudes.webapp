	...
	<div align=center>
		...
		<div class="genericSearchArea wideFields" align=center>
			<form id="searchUserForm">
				<table>
					<tr>
						<td class=rightAlignColumns><label for="userNameSearchField"><spring:message
									code='User_Name' />:</label></td>
						<td><input id="username_filter" type="text" name="username" /></td>
						<td>
						<td class=rightAlignColumns><label for="statusSearchField"><spring:message
									code='Status' />:</label></td>
						<td><select id="status_filter" type="text" name="status">
						</select></td>

					</tr>
					<tr>
						<td class=rightAlignColumns><label for="statusSearchField"><spring:message
									code='user_n4' />:</label></td>
						<td><input id="usern4_filter" type="text" id="userN4" /> <input
							class="" id="userN4_id_vali" name="userN4_id_vali" type="hidden" />
						</td>
						<td>
						<td class=rightAlignColumns><label for="nivelSearchField"><spring:message
									code='Report_level' />:</label></td>
						<td><select id="nivel_filter" type="text"
							name="reportAccessLevel"></select></td>
					</tr>
					<tr>
						<td class=rightAlignColumns><label for="fechaSearchField"><spring:message
									code='Fecha_final' />:</label></td>
						<td><input id="fecha_filter" type="text" name="validity_date"></input>
						</td>
						<td>
						<td class=rightAlignColumns><label for="emailSearchField"><spring:message
									code='Email' />:</label></td>
						<td><input id="email_filter" type="text" name="email" /></td>
						<td>
					</tr>
					<tr>
						<td class=rightAlignColumns><label for="roleSearchField"><spring:message
									code='Profile' />:</label></td>
						<td><select id="profile_filter" name="profile">
						</select></td>
						<td>
						<td class=rightAlignColumns><label for="roleSearchField"><spring:message
									code='Empresa_' />:</label></td>
						<td><select id="empresa_filter" name="empresa">
						</select></td>
					</tr>
					<tr>
					<td class=rightAlignColumns><label for="roleSearchField" id="pantalla"><spring:message
									code='pantalla' />:</label></td>
						<td><select id="pantalla_filter" name="pantalla">
						</select></td>
					</tr>
					
				</table>
				<br /> <input type="submit" value="<spring:message code='Search' />" />
			</form>
		</div>
		<br />
		...
		<div class="genericButtonsArea wideFields" align=center>
			<input id=newUserButton type=button
				value="<spring:message code='NewUser' />" /> <input
				id=editUserButton type=button
				value="<spring:message code='EditUser' />" /> <input
				id=deleteUserButton type=button
				value="<spring:message code='DeleteUser' />" />
		</div>
	</div>
...
<script type="text/javascript">
	...
	var noSel = {
		id : "",
		name : "<spring:message code='_No_Selection_' />"
	};
	...
	var pantallaList = (<c:out value='${pantallaList}' escapeXml="false" />);

	//execute loaded function when page loaded
	jQuery(document).ready(loaded);

	function loaded() {
		jQuery("#fecha_filter").datepicker({
			dateFormat : 'yy-mm-dd'
		});
		...
		jQuery("#usern4_filter")
				.autocomplete(
						{
							source : function(req, resp) {
								blockPage();
								var prevUrl = "";
								if ($("#profile_filter option:selected").text() === "Transportista")
									prevUrl = 'rest/Transportista/likes/';
								else
									prevUrl = 'rest/UserN4/like.json?results=10&match=';
								jQuery.getJSON(prevUrl
							...
							select : function(event, ui) {
								jQuery("#userN4_id_vali").val(ui.item.key);
								//alert(ui.item.key);
							}
						});

		fillSelect(jQuery("#status_filter"), [ noSel ].concat(statusList),
				function(eachObj) {
					return {
						value : eachObj.id,
						text : eachObj.name
		...
		fillSelect(jQuery("#pantalla_filter"), [ noSel ].concat(pantallaList),
				function(eachObj) {
					return {
						value : eachObj.id,
						text : eachObj.name
		...
		fillSelect(jQuery("#profile_filter"), [ noSel ].concat(profileList),
				function(eachObj) {
					return {
						value : eachObj.id,
						text : eachObj.name
		...
		fillSelect(jQuery("#empresa_filter"), [ noSel ].concat(empresaList),
				function(eachObj) {
					return {
						value : eachObj.id,
						text : eachObj.nombre
		...
		fillSelect(jQuery("#nivel_filter"), [ noSel ]
				.concat(reportAccessLevelList), function(eachObj) {
			return {
				value : eachObj.id,
				text : eachObj.name
			};
		...
		jQuery("#userAdminContainer").load(
				User_Panel.uiFile + "?ts=" + new Date().getTime(),
				function() {
					userPanel = new User_Panel();
					userPanel.registerHandler(User_Panel.Events.SAVE_USER,
							function(sender, object) {
								var requestInfo = {
									id : object.id
								};
								updateTableWithRequest(getUserTable(),
										"user-admin/user-table-row.json",
										requestInfo);
							});
					...
					userPanel.fillpantalla(pantallaList);
	...
	function deleteUserClicked() {
		...
		showAcceptCancel("<spring:message code='confirm_delete_user' />",
				deleteUser);
	}

	function deleteUser() {
		// send the information to the server, acceptNewUser as callback
		jQuery.postJSON("user-admin/delete-user.json", {
			id : userId
		}, deleteUser_callback);
		...
		getUserTable().jqGrid(
				{
					...
					datatype : "json",
					// column names
					colNames : [ "<spring:message code='User_Id' />",
							...
							"<spring:message code='Empresa_' />",
							"<spring:message code='Report_level' />",
							"<spring:message code='Fecha_final' />",
							"<spring:message code='user_n4' />"],
					// column properties
					colModel : [ {
						name : 'id',
						index : 'id',
	...
	function editUserClicked() {
		...
		jQuery.postJSON("user-admin/edit-user.json", {
			id : userId
