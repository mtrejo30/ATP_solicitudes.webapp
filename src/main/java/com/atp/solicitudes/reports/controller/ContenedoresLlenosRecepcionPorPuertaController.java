...
@Controller
@RequestMapping ("report_contenedores_llenos_recepcion_por_puerta")
public class ContenedoresLlenosRecepcionPorPuertaController extends LocalBaseController
{
	/**
	 * Field logger.
	 */
	protected static Logger logger = LoggerFactory.getLogger(ContenedoresLlenos_RecepcionPorPuerta.class);
	...
	public String show(Map<String, Object> model, HttpSession session)
	{
		try
		{
			...
			return "report_contenedores_llenos_recepcion_por_puerta";
		}
		catch (Exception ex)
		{
	...
	public Object reportTableInfoRequest(HttpServletRequest request)
	{
		...
		DaoResult<ContenedoresLlenos_RecepcionPorPuerta> col = getObjects(request);
        ...
        col.setPageNumber(pageNumber);
        col.setNumberOfRows(numberOfRows);
	...
	private DaoResult<ContenedoresLlenos_RecepcionPorPuerta> getObjects(HttpServletRequest request)
	{
		DaoResult<ContenedoresLlenos_RecepcionPorPuerta> col = new DaoResult<ContenedoresLlenos_RecepcionPorPuerta>();
		Integer pageNumber = Integer.parseInt(request.getParameter(PAGE_PARAMETER));
		Integer numberOfRows = Integer.parseInt(request.getParameter(ROWS_PARAMETER));
		...
		try
		{
			...
			if (digitoVerificador.length() > 0)
				col = domainManager.getContenedoresLlenos_RecepcionPorPuerta(digitoVerificador);
		}
		catch (Exception e)
		{
	...
	private ArrayNode getNodes(List<ContenedoresLlenos_RecepcionPorPuerta> col)
	{
		...
		for (ContenedoresLlenos_RecepcionPorPuerta eachObj : col)		
		{
			...
			eachArray.add(formatter.format(eachObj.getFechaDeIngreso()));
