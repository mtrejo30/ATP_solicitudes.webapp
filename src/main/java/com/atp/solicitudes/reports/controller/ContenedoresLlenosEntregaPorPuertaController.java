...
@Controller
@RequestMapping ("report_contenedores_llenos_entrega_por_puerta")
public class ContenedoresLlenosEntregaPorPuertaController extends LocalBaseController
{
	/**
	 * Field logger.
	 */
	protected static Logger logger = LoggerFactory.getLogger(ContenedoresLlenos_EntregaPorPuerta.class);
	...
	public String show(Map<String, Object> model, HttpSession session)
	{
		try
		{
			...
			return "report_contenedores_llenos_entrega_por_puerta";
		}
		catch (Exception ex)
		{
	...
	public Object reportTableInfoRequest(HttpServletRequest request)
	{
		...
		DaoResult<ContenedoresLlenos_EntregaPorPuerta> col = getObjects(request);
	...
	private DaoResult<ContenedoresLlenos_EntregaPorPuerta> getObjects(HttpServletRequest request)
	{
		DaoResult<ContenedoresLlenos_EntregaPorPuerta> col = new DaoResult<ContenedoresLlenos_EntregaPorPuerta>();
		Integer pageNumber = Integer.parseInt(request.getParameter(PAGE_PARAMETER));
		Integer numberOfRows = Integer.parseInt(request.getParameter(ROWS_PARAMETER));
		...
		try
		{
			...
			if (digitoVerificador.length() > 0)
				col = domainManager.getContenedoresLlenos_EntregaPorPuerta(digitoVerificador);
		}
		catch (Exception e)
		{
	...
	private ArrayNode getNodes(List<ContenedoresLlenos_EntregaPorPuerta> col)
	{
		...
		for (ContenedoresLlenos_EntregaPorPuerta eachObj : col)		
		{
			...
			eachArray.add(formatter.format(eachObj.getFechaDeSalida()));
