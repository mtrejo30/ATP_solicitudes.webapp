...
@Controller
@RequestMapping ("report_consultas_por_bl")
public class ConsultaBLController extends LocalBaseController
{
	/**
	 * Field logger.
	 */
	protected static Logger logger = LoggerFactory.getLogger(ConsultaBLController.class);
	...
	public String show(Map<String, Object> model, HttpSession session)
	{
		try
		{
			...
			return "report_consultas_por_bl";
		}
		catch (Exception ex)
		{
	...
	public Object reportTableInfoRequest(HttpServletRequest request)
	{
		...
		DaoResult<ConsultaBL> col = getObjects(request);
	...
	private DaoResult<ConsultaBL> getObjects(HttpServletRequest request)
	{
		DaoResult<ConsultaBL> col = new DaoResult<ConsultaBL>();
		//Integer pageNumber = Integer.parseInt(request.getParameter(PAGE_PARAMETER));
		//Integer numberOfRows = Integer.parseInt(request.getParameter(ROWS_PARAMETER));
       ...
       // col.setPageNumber(pageNumber);
		...
		try
		{
			...
			String bl = node.get("inputBL").asText().trim();

			if (bl.length() > 0)
				col = domainManager.getConsultaBL(bl);
		}
		catch (Exception e)
		{
	...
	private ArrayNode getNodes(List<ConsultaBL> col)
	{
