...
@Controller
@RequestMapping ("report_contenedores_llenos_ultimo_embarque")
public class ContenedoresLlenosUltimoEmbarqueController extends LocalBaseController
{
	/**
	 * Field logger.
	 */
	protected static Logger logger = LoggerFactory.getLogger(ContenedoresLlenosUltimoEmbarqueController.class);
	...
	public String show(Map<String, Object> model, HttpSession session)
	{
		try
		{
			...
			return "report_contenedores_llenos_ultimo_embarque";
		}
		catch (Exception ex)
		{
	...
	public Object reportTableInfoRequest(HttpServletRequest request)
	{
		...
		DaoResult<ContenedoresLlenos_UltimoEmbarque> col = getObjects(request);
	...
	private DaoResult<ContenedoresLlenos_UltimoEmbarque> getObjects(HttpServletRequest request)
	{
		DaoResult<ContenedoresLlenos_UltimoEmbarque> col = new DaoResult<ContenedoresLlenos_UltimoEmbarque>();
		Integer pageNumber = Integer.parseInt(request.getParameter(PAGE_PARAMETER));
		Integer numberOfRows = Integer.parseInt(request.getParameter(ROWS_PARAMETER));
		...
		try
		{
			...
			if (digitoVerificador.length() > 0)
				col = domainManager.getContenedoresLlenos_UltimoEmbarque(digitoVerificador);
		}
		catch (Exception e)
		{
	...
	private ArrayNode getNodes(List<ContenedoresLlenos_UltimoEmbarque> col)
	{
		...
		for (ContenedoresLlenos_UltimoEmbarque eachObj : col)		
		{
			...
			eachArray.add(formatter.format(eachObj.getFechaDeEmbarque()));
