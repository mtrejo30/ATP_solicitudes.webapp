...
public class ConsultaOperativoController extends LocalBaseController
{
	...
	{
		//DaoResult<ConsultaOperativo> query = getObjects(request);
        ...
        //int a= 1;
        //Long myLong= new Long(a);
		//col.setTotalRows(myLong);
	...
	private DaoResult<ConsultaOperativo> getObjects(HttpServletRequest request)
	{
		...
		try
		{
			...
			{
				col = domainManager.getConsultaOperativoExportacion(buqueViaje);
			}
		}
		catch (Exception e)
		{
	...
	private ArrayNode getNodes(List<ConsultaOperativo> col)
	{
		...
		//DateFormat formatter = (DateFormat) getBean("dateFormatter_yyyy-MM-dd_hh_mm_ss");CAMBIO FORMATO FECHA
		...
		for (ConsultaOperativo eachObj : col)		
		{
			...
			eachArray.add(eachObj.getFechaLiberado());
