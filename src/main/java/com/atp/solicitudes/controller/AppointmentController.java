...
import com.atp.solicitudes.manager.impl.DomainManagerN4Impl;
...
public class AppointmentController extends LocalBaseController
{
	...
	public String show(Map<String, Object> model, HttpServletRequest request)
	{
		...
		try
		{
			...
			List<SolicitudAppointment> col = null;
			//set nivel transporter			
			model.put("perfil", loggedUser.getProfile().getCode());
			if(loggedUser.getProfile().getCode().equals("TRA")){
				col = manager.getSolicitudAppointmentFromSolicitudByTransporter(solicitud,loggedUser.getUsuarioN4_id());
				...
				if(objNivel != null)
					model.put("nivelTransporter", objNivel.getNivelString());
					
				else
					model.put("nivelTransporter","Ninguno");
			}else{					
			col = manager.getSolicitudAppointmentFromSolicitud(solicitud);
			}
			...
			if(col !=null  && col.size() >0)
			{
				...
				{
					if(!transporterRows.contains(each.getTransportistaId()+"_"+each.getOperadorId()+"_"+each.getPlacas()+"_"+each.getDateScheduled().toString()+"_"+ each.getContenedor()))
					{
						transporterRows.add(each.getTransportistaId()+"_"+each.getOperadorId()+"_"+each.getPlacas()+"_"+each.getDateScheduled().toString()+"_"+ each.getContenedor());
					}
				}
				...
				if(transporterRows.size()>0)
				{
					...
					for(String eachTransporterRow : transporterRows)
					{
						...
						String stringidpadre_ = arrayTransporterRow[4];
						...
						aTransporter.put("idpadre_", stringidpadre_);
						...
						{
							if((eachAppoinment.getTransportistaId()+"_"+eachAppoinment.getOperadorId()+"_"+eachAppoinment.getPlacas()+"_"+eachAppoinment.getDateScheduled().toString()+"_"+eachAppoinment.getContenedor()).equals(eachTransporterRow))
					...
					model.put("transporters", allTransporters);//regresa todo los valores de transporter a la vista
				}
...
//					}
	...
	public Object acceptGeneralInformation(@RequestBody JsonNode node, HttpServletRequest request) throws DomainModelException
	{
		...
		try
		{
			...
			String contenedor = node.get("contenedor").asText();
			if(solicitud.getOperationType().toString().equals("IMPO"))
			{
				 identificador = node.get("booking").asText();
			}
		//	String bookingpedimentos =  node.get("booking").asText();
			
			DateFormat format = (DateFormat) getBean("dateFormatter_yyyy-MM-dd_hh_mm_ss");
			Date scheduleDate = format.parse(dateStr + " " + timeStr);
            System.out.print("contenedor en forma de id"+contenedor);
...
//			builder.append(getMessageFromResource("Appointment_registered_please_verify_data_matches_what_is_requested"));
			...
			for (JsonNode each : appointments)
			{
				...
				try
				{
					...
					//TODO
					//manda al guardar donde capture la fecha del xml
					...
					appointment.setContenedor(contenedor);
					if(solicitud.getOperationType().toString().equals("IMPO"))
					{
						appointment.setPedimento_n4(identificador);
					}
...
//	}
	...
	public Object getTimeSlots(@RequestBody JsonNode node, HttpServletRequest request) throws Exception
	{
		...
		Solicitud solicitud = (Solicitud) session.getAttribute(Solicitud.SLOT_NAME);
		String stit;
		try{ stit = node.get("tit").asText();}catch(Exception e){stit = null;}
 		try
		{
			Date aDate = formatter.parse(node.get("date").asText());
			
			//TODO 
			List<Date> dates = managerN4.getTimeSlotsForDate(aDate, solicitud,stit);
