...
import com.atp.solicitudes.model.ReportAccessLevel;
...
import com.atp.solicitudes.model.UserPantallaEnum;
...
public class UserAdminController extends LocalBaseController
{
	...
	public String show(Map<String, Object> model, HttpSession session)
	{
		try
		{
			...
			model.put("pantallaList", JSONUtils.getJSONFromObject(UserPantallaEnum.getValidDisplayValues()));
	...
	private DaoQuery getTableQuery(HttpServletRequest request)
	{
		...
		try
		{
			...
			addStringParameterToQuery(query, node, "userN4_id_vali", "userN4_id_vali");
			addStringParameterToQuery(query, node, "validity_date", "validity_date");
			addStringParameterToQuery(query, node, "email", "email");
			addObjectParameterToQuery(query, node, "pantalla", "pantalla", new UserPantallaEnum.IdResolver());
			...
			addObjectParameterToQuery(query, node, "empresa", "empresa", new Empresa.IdResolver());
			addObjectParameterToQuery(query, node, "reportAccessLevel", "reportAccessLevel", new ReportAccessLevel.IdResolver());
		}
		catch (Exception e)
		{
	...
	private ArrayNode getUserNodes(List<User> users)
	{
		...
		for (User eachUser : users)		
		{
			...
			eachArray.add(eachUser.getEmpresa().getNombre());
			eachArray.add(eachUser.getReportAccessLevel().getName());
			eachArray.add(eachUser.getvalidity_date());
			eachArray.add(eachUser.getuserN4());
	...
	public Object getUserTransportistasByUserN4(@PathVariable(value = "id") String id)
	{
		...
		try
		{
			user = domainManager.getUserWithUsername(id);
		}
		catch (Exception ex)
		{
