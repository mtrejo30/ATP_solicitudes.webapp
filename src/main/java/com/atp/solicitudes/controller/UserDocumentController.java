...
public class UserDocumentController extends LocalBaseController
{
	...
	public String acceptFile(UserDocumentForm userDocumentForm, Map<String, Object> model, HttpServletRequest request)
	{
		...
		try
		{
            ...
            if(userDocumentForm.getType() ==0)
            {
            	if(userDocumentForm.getBltxt().equals("No Aplica"))
            	{
            		
            		throw new  DomainModelException ("capturar Bls");
            	}
            }
            if(userDocumentForm.getType() ==1)
            {
            	if(userDocumentForm.getPedimentotxt().equals("No Aplica"))
            	{
            		
            		throw new  DomainModelException ("capturar pedimentos");
            	}
            }
			...
			documentoSolicitud.setBls(userDocumentForm.getBltxt());
			documentoSolicitud.setPedimentos(userDocumentForm.getPedimentotxt());
			documentoSolicitud.setLinea_naviera(userDocumentForm.getLinea());	
			documentoSolicitud.setFerrocaril(userDocumentForm.getFerrocaril());
	...
	private ArrayNode getTableNodes(List<DocumentoSolicitud> docs)
	{
		...
		for (DocumentoSolicitud eachDocument : docs)
		{
			...
			eachArray.add(eachDocument.getPedimentos());
			eachArray.add(eachDocument.getBls());
			eachArray.add(eachDocument.getLinea_naviera());
			eachArray.add(eachDocument.getFerrocaril());
	...
	@RequestMapping(value = "/get-document-appoiment/{id}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public Object getId(@PathVariable(value = "id") Integer id)
	{
		...
		try
		{
			//listDocs = domainManager.get;
			listDocs  = domainManager.getDocumentoSolicitudbyId1(id);
