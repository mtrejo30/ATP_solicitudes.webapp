...
//		Integer countContainers=null;
		if (node.has("contenedor"))
		{
			...
			String dateStr1 = node.get("dayScheduled").asText();
			String timeStr1 = node.get("timeScheduled").asText();
			DateFormat format1 = (DateFormat) getBean("dateFormatter_yyyy-MM-dd_hh_mm_ss_T");
			Date scheduleDate1 = format.parse(dateStr + "  " + timeStr);
//			countContainers=getCountUnitNbr(node.get("contenedor").get("label").asText());
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
			sdf.setTimeZone(TimeZone.getTimeZone("GMT"));
			String date = sdf.format(new Date());
			XMLGregorianCalendar xmlCal = DatatypeFactory.newInstance().newXMLGregorianCalendar(date);ContainerType container = new ContainerType();
