...
public class DocumentoSolicitudDaoHibernate extends HibernateDaoTemplate implements DocumentoSolicitudDao
{
	...
	public List<String> getDocumentsWithId(Integer id) throws Exception
	{
		List<String> listDocs = new ArrayList<String>();
		Criteria criteria = getCriteria();
		criteria.add(Restrictions.eq("solicitud.folio", id));
		...
		if (col.size() == 0)
			...
			for (int i = 0; i < col.size(); i++) {
				listDocs.add(col.get(i).getSolicitud().getFolio().toString());
			}
			return listDocs;
		}
