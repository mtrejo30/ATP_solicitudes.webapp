...
public class ContenedorDaoMyBatis extends MyBatisDaoTemplate implements ContenedorDao
{
	...
	public DaoResult<ContenedorStatus> getContenedorStatus(String numContenedor) throws Exception{
		DaoResult<ContenedorStatus> result = new DaoResult<ContenedorStatus>();
		
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("numContenedor", numContenedor.substring(0, numContenedor.length() - 1) + "%");
		
		List<ContenedorStatus> col = 
				this.<ContenedorStatus> selectList("mybatis.reports.ContenedorStatusMapper.selectContenedorStatus", map, new RowBounds()); 
		result.setCollection(col);
